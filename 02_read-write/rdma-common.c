...
struct connection {
    ...
    enum { SS_INIT, SS_MR_SENT, SS_RDMA_SENT, SS_DONE_SENT } send_state;

    enum { RS_INIT, RS_MR_RECV, RS_DONE_RECV } recv_state;
};
...
void build_context(struct ibv_context *verbs) {
    ...
    TEST_Z(s_ctx->cq = ibv_create_cq(s_ctx->ctx, 10, NULL, s_ctx->comp_channel,
...
void on_completion(struct ibv_wc *wc) {
    ...
    if (wc->opcode & IBV_WC_RECV) {
        ...
        if (conn->recv_msg->type == MSG_MR) {
            memcpy(&conn->peer_mr, &conn->recv_msg->data.mr,
                   sizeof(conn->peer_mr));
            post_receives(conn); /* only rearm for MSG_MR */

            if (conn->send_state ==
                SS_INIT) /* received peer's MR before sending ours, so send ours
                            back */
                send_mr(conn);
        }
...
void register_memory(struct connection *conn) {
    ...
    TEST_Z(conn->send_mr = ibv_reg_mr(s_ctx->pd, conn->send_msg,
    ...
    TEST_Z(conn->rdma_local_mr =
               ibv_reg_mr(s_ctx->pd, conn->rdma_local_region, RDMA_BUFFER_SIZE,
                          ((s_mode == M_WRITE) ? 0 : IBV_ACCESS_LOCAL_WRITE)));

    TEST_Z(conn->rdma_remote_mr = ibv_reg_mr(
               s_ctx->pd, conn->rdma_remote_region, RDMA_BUFFER_SIZE,
               ((s_mode == M_WRITE)
...
void send_mr(void *context) {
    ...
    memcpy(&conn->send_msg->data.mr, conn->rdma_remote_mr,
           sizeof(struct ibv_mr));

    send_message(conn);
}

void set_mode(enum mode m) { s_mode = m; }
